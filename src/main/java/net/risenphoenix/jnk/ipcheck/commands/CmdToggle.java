		...
		if (sender.hasPermission("ipcheck.toggle") || sender.isOp()) {
			if (args.length == 2) {
				...
				if (args[1].equalsIgnoreCase("login-notify") || args[1].equalsIgnoreCase("notification") || args[1].equalsIgnoreCase("notify")) {
					int response = IPcheck.Instance.Configuration.toggle(0);
					
					if (response == 0) {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.YELLOW + IPcheck.Instance.Translation.getTranslation("TOGGLE_NOTIFY") + ChatColor.RED + "False");
					} else if (response == 1) {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.YELLOW + IPcheck.Instance.Translation.getTranslation("TOGGLE_NOTIFY") + ChatColor.GREEN + "True");
					} else {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.RED + IPcheck.Instance.Translation.getTranslation("TOGGLE_ERR"));
					}
					
				} else if (args[1].equalsIgnoreCase("detail-notify") || args[1].equalsIgnoreCase("detail") || args[1].equalsIgnoreCase("dn")) {
					int response = IPcheck.Instance.Configuration.toggle(1);
					
					if (response == 0) {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.YELLOW + IPcheck.Instance.Translation.getTranslation("TOGGLE_DETAIL") + ChatColor.RED + "False");
					} else if (response == 1) {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.YELLOW + IPcheck.Instance.Translation.getTranslation("TOGGLE_DETAIL") + ChatColor.GREEN + "True");
					} else {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.RED + IPcheck.Instance.Translation.getTranslation("TOGGLE_ERR"));
					}
					
				} else if (args[1].equalsIgnoreCase("secure-mode") || args[1].equalsIgnoreCase("secure") || args[1].equalsIgnoreCase("sm")) {
					int response = IPcheck.Instance.Configuration.toggle(2);
					
					if (response == 0) {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.YELLOW + IPcheck.Instance.Translation.getTranslation("TOGGLE_SECURE") + ChatColor.RED + "False");
					} else if (response == 1) {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.YELLOW + IPcheck.Instance.Translation.getTranslation("TOGGLE_SECURE") + ChatColor.GREEN + "True");
					} else {
						sender.sendMessage(ChatColor.GOLD + IPcheck.PLUG_NAME + ChatColor.RED + IPcheck.Instance.Translation.getTranslation("TOGGLE_ERR"));
					}
					...
					sender.sendMessage(IPcheck.Instance.Translation.getTranslation("ILL_ARGS_ERR"));
				}
			} else {
