            ...
            if (!IPcheck.Database.isExemptedPlayer(player.getName()) && !IPcheck.Database.isExemptedIP(ip)) { // If number of accounts is greater than threshold and player is not exempt
                ...
                for(int i = 0; i < online.length; i++) {
                        if (IPcheck.Configuration.detailNotify) { // If Detailed Notifications are enabled, display the detailed notification.
                            ...
                            if (online[i].hasPermission("ipcheck.getnotify") || online[i].isOp()) { // If player is an operator or has the ipcheck.getnotify permission
                                    online[i].sendMessage(ChatColor.DARK_GRAY + "---------------------------------------");
                                    online[i].sendMessage(ChatColor.GOLD + TranslationManager.PLUG_NAME + ChatColor.RED + "Warning! " + ChatColor.LIGHT_PURPLE + player.getDisplayName() + ChatColor.RED + " may have multiple accounts!");
                                    online[i].sendMessage(ChatColor.DARK_GRAY + "---------------------------------------");
                            }
                        }
                    }
